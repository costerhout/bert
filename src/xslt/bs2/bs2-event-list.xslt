...
@Last modified time: 2016-08-26T16:55:59-08:00
...
<xsl:stylesheet
    ...
    <xd:doc>
        <xd:short>Create a table row with information or a single event.</xd:short>
        <xd:detail>
            <p>Each event will have a separate row in the table with information on the date, time, event name, and location displayed.  Event information is available via a modal window, and a location map is available via a separate modal.</p>
        </xd:detail>
    </xd:doc>
    <xsl:template match="Event" mode="modal">
        ...
        <xsl:variable name="idModalLocation" select="concat(generate-id(), '-location-modal')"/>
        <xsl:variable name="idModalLocationTitle" select="concat(generate-id(), '-location-title')"/>
        ...
        <xsl:variable name="rtfModalDescriptionBody">
            ...
            <xsl:if test="image[@type='file']">
                <img src="image/path" alt="" class="event-thumbnail"/>
            </xsl:if>
    ...
    <xsl:template match="DateTime">
        ...
        <tr>
            ...
            <td><a href="{concat('#', $idModalDescription)}" title="{concat('Open up description for event: ', parent::Event/Event_Name)}"><xsl:value-of select="parent::Event/Event_Name"/></a></td>
            <td>
                <!-- If we have pinpointed a location then set the location as a link to popup the modal map -->
                <xsl:choose>
                    <xsl:when test="$sLocation != ''">
                        <a href="{concat('#', $idModalLocation)}" title="{concat('Open up map for event: ', parent::Event/Event_Name)}"><xsl:value-of select="$sLocationText"/></a>
                    </xsl:when>
                    <xsl:otherwise>
